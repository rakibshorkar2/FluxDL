...
final class BrowserTests: XCTestCase {
    ...
    // MARK: - Multi-tab isolation
    
    /// Resets the shared manager to a single fresh home tab and returns its id.
    @MainActor
    @discardableResult
    private func resetTabManager() -> UUID {
        let tabManager = BrowserTabManager.shared
        tabManager.closeAllTabs()
        return tabManager.activeTabId
    }
    
    /// Creating multiple tabs produces unique IDs — no two tabs may ever
    /// share an identity.
    @MainActor
    func testMultiTabCreationProducesUniqueIDs() {
        let tabManager = BrowserTabManager.shared
        resetTabManager()
        
        let a = tabManager.createNewTab()
        let b = tabManager.createNewTab()
        let c = tabManager.createNewTab()
        
        XCTAssertNotEqual(a, b)
        XCTAssertNotEqual(b, c)
        XCTAssertNotEqual(a, c)
        XCTAssertEqual(Set(tabManager.tabs.map(\.id)).count, tabManager.tabs.count)
    }
    
    /// Each tab preserves its own URL regardless of which tab is selected.
    @MainActor
    func testEachTabPreservesItsOwnURL() {
        let tabManager = BrowserTabManager.shared
        resetTabManager()
        
        let a = tabManager.createNewTab(url: URL(string: "https://apple.com"))
        let b = tabManager.createNewTab(url: URL(string: "https://github.com"))
        let c = tabManager.createNewTab(url: URL(string: "https://wikipedia.org"))
        
        XCTAssertEqual(tabManager.tab(for: a)?.url, URL(string: "https://apple.com"))
        XCTAssertEqual(tabManager.tab(for: b)?.url, URL(string: "https://github.com"))
        XCTAssertEqual(tabManager.tab(for: c)?.url, URL(string: "https://wikipedia.org"))
        
        tabManager.selectTab(id: a)
        tabManager.selectTab(id: c)
        tabManager.selectTab(id: b)
        
        XCTAssertEqual(tabManager.tab(for: a)?.url?.absoluteString, "https://apple.com")
        XCTAssertEqual(tabManager.tab(for: b)?.url?.absoluteString, "https://github.com")
        XCTAssertEqual(tabManager.tab(for: c)?.url?.absoluteString, "https://wikipedia.org")
    }
    
    /// Selecting tab A must never mutate tab B's state — this is the exact
    /// contract every WebKit callback now follows (bound to its own tabID).
    @MainActor
    func testSelectingTabDoesNotMutateOtherTabs() {
        let tabManager = BrowserTabManager.shared
        resetTabManager()
        ...
        // Simulate tab B's own web view reporting progress/title/URL.
        tabManager.updateTab(id: b) { tab in
            tab.title = "B's Page"
            tab.url = URL(string: "https://b.com/details")
            tab.inputURLText = "https://b.com/details"
        ...
        let tabA = tabManager.tab(for: a)
        let tabB = tabManager.tab(for: b)
        XCTAssertEqual(tabA?.url?.absoluteString, "https://a.com")
        XCTAssertEqual(tabA?.title, "New Tab")
        // Tab B retains its own mutated state; nothing leaked to tab A.
        XCTAssertEqual(tabB?.title, "B's Page")
        XCTAssertEqual(tabB?.url?.absoluteString, "https://b.com/details")
        XCTAssertEqual(tabB?.estimatedProgress, 0.75)
        XCTAssertTrue(tabB?.isLoading == true)
        XCTAssertTrue(tabB?.canGoBack == true)
    }
    
    /// Tab order stays stable across switching.
    @MainActor
    func testTabOrderRemainsCorrectAfterSwitching() {
        let tabManager = BrowserTabManager.shared
        let baseID = resetTabManager()
        
        let a = tabManager.createNewTab(url: URL(string: "https://a.com"))
        let b = tabManager.createNewTab(url: URL(string: "https://b.com"))
        let c = tabManager.createNewTab(url: URL(string: "https://c.com"))
        
        for id in [a, b, c, b, a, c] {
            tabManager.selectTab(id: id)
        }
        
        XCTAssertEqual(tabManager.tabs.map(\.id), [baseID, a, b, c])
    }
    
    /// Closing one tab leaves the remaining tabs' state and order intact.
    @MainActor
    func testClosingOneTabDoesNotCorruptRemainingTabs() {
        let tabManager = BrowserTabManager.shared
        let baseID = resetTabManager()
        ...
        XCTAssertEqual(tabManager.tabs.map(\.id), [baseID, a, c])
        XCTAssertEqual(tabManager.tab(for: a)?.url?.absoluteString, "https://a.com")
        XCTAssertEqual(tabManager.tab(for: c)?.url?.absoluteString, "https://c.com")
        XCTAssertEqual(tabManager.activeTabId, a)
        XCTAssertNil(tabManager.tab(for: b))
    }
    
    /// Rapid switching among 3+ tabs never produces an invalid active ID.
    @MainActor
    func testRepeatedSwitchingAmongThreeTabsStaysValid() {
        let tabManager = BrowserTabManager.shared
        resetTabManager()
        
        let ids = [
            tabManager.createNewTab(url: URL(string: "https://a.com")),
            tabManager.createNewTab(url: URL(string: "https://b.com")),
            tabManager.createNewTab(url: URL(string: "https://c.com"))
        ]
        
        for round in 0..<20 {
            for id in ids {
                tabManager.selectTab(id: id)
                XCTAssertEqual(tabManager.activeTabId, id, "round \(round)")
                XCTAssertNotNil(tabManager.activeTab, "active tab must resolve at round \(round)")
                XCTAssertTrue(tabManager.tabs.contains { $0.id == tabManager.activeTabId })
            }
        }
    ...
    func testRestoredTabsRetainIndependentURLs() {
        ...
        var tab1 = BrowserTabModel(title: "One", url: URL(string: "https://one.com/page1"))
        var tab2 = BrowserTabModel(title: "Two", url: URL(string: "https://two.com/page2"))
        tab2.isDesktopMode = true
        persistence.save(tabs: [tab1, tab2], activeTabId: tab1.id)
        ...
        let tabs = restored?.tabs.map { BrowserTabModel(snapshot: $0) } ?? []
        XCTAssertEqual(tabs.count, 2)
        XCTAssertEqual(tabs[0].url?.absoluteString, "https://one.com/page1")
        XCTAssertEqual(tabs[1].url?.absoluteString, "https://two.com/page2")
        XCTAssertEqual(tabs[0].title, "One")
        XCTAssertEqual(tabs[1].title, "Two")
        XCTAssertFalse(tabs[0].isDesktopMode)
        XCTAssertTrue(tabs[1].isDesktopMode)
        XCTAssertEqual(restored?.activeTabId, tab1.id)
        // Restored tabs get their web views lazily — never serialized.
        XCTAssertTrue(tabs.allSatisfy { $0.webView == nil })
    }
    
    // MARK: - Torrent link classification
    
    /// `magnet:` URIs are recognized as torrent links.
    @MainActor
    func testMagnetLinkRecognizedAsTorrent() {
        let magnet = URL(string: "magnet:?xt=urn:btih:0123456789abcdef0123456789abcdef01234567&dn=Movie&tr=http://t.example.com/announce")!
        XCTAssertTrue(BrowserTorrentLink.isMagnet(magnet))
        XCTAssertTrue(BrowserTorrentLink.isTorrent(magnet))
        XCTAssertFalse(BrowserTorrentLink.isRemoteTorrent(magnet))
        
        // Case-insensitive scheme.
        let upper = URL(string: "MAGNET:?xt=urn:btih:0123456789abcdef0123456789abcdef01234567")!
        XCTAssertTrue(BrowserTorrentLink.isMagnet(upper))
    }
    
    /// Remote `.torrent` URLs are recognized as torrent links.
    @MainActor
    func testTorrentURLsRecognizedAsTorrent() {
        XCTAssertTrue(BrowserTorrentLink.isRemoteTorrent(URL(string: "https://example.com/movie.torrent")!))
        XCTAssertTrue(BrowserTorrentLink.isTorrent(URL(string: "http://cdn.example.com/linux.iso.torrent")!))
        XCTAssertTrue(BrowserTorrentLink.isTorrent(URL(string: "https://example.com/movie.TORRENT")!))
        XCTAssertFalse(BrowserTorrentLink.isTorrent(URL(string: "https://example.com/movie.torrent.html")!))
    }
    
    /// Normal downloadable files keep using the generic download flow.
    @MainActor
    func testNormalDownloadsAreNotTorrents() {
        for ext in ["zip", "mp4", "pdf", "rar", "7z", "dmg", "mkv", "apk"] {
            let url = URL(string: "https://example.com/file.\(ext)")!
            XCTAssertFalse(BrowserTorrentLink.isTorrent(url), "\(ext) must not be a torrent link")
            XCTAssertFalse(BrowserTorrentLink.isMagnet(url))
            XCTAssertFalse(BrowserTorrentLink.isRemoteTorrent(url))
        }
        XCTAssertFalse(BrowserTorrentLink.isTorrent(URL(string: "https://example.com/page")!))
        XCTAssertFalse(BrowserTorrentLink.isTorrent(URL(string: "https://example.com/torrents")!))
        
        // A normal zip still reaches the generic download popup.
        let viewModel = BrowserViewModel()
        viewModel.promptDownload(url: URL(string: "https://example.com/file.zip")!)
        XCTAssertTrue(viewModel.showDownloadPrompt)
        XCTAssertFalse(viewModel.showTorrentPrompt)
    }
    
    /// Magnet metadata extraction: dn, info hash, tracker count.
    @MainActor
    func testMagnetMetadataExtraction() {
        let magnet = URL(string: "magnet:?xt=urn:btih:0123456789abcdef0123456789abcdef01234567&dn=My+Movie+2026&xl=123456&tr=http%3A%2F%2Ft1.example.com%2Fannounce&tr=http://t2.example.com/announce")!
        
        XCTAssertEqual(BrowserTorrentLink.displayName(from: magnet), "My Movie 2026")
        XCTAssertEqual(BrowserTorrentLink.infoHash(from: magnet), "0123456789abcdef0123456789abcdef01234567")
        XCTAssertEqual(BrowserTorrentLink.trackerCount(in: magnet), 2)
        
        let viewModel = BrowserViewModel()
        viewModel.promptTorrent(url: magnet)
        XCTAssertTrue(viewModel.showTorrentPrompt)
        XCTAssertEqual(viewModel.torrentPrompt?.kind, .magnet)
        XCTAssertEqual(viewModel.torrentPrompt?.displayName, "My Movie 2026")
        XCTAssertEqual(viewModel.torrentPrompt?.infoHash, "0123456789abcdef0123456789abcdef01234567")
        XCTAssertEqual(viewModel.torrentPrompt?.trackerCount, 2)
        XCTAssertEqual(viewModel.torrentPrompt?.kindLabel, "Magnet Link")
        
        // The raw magnet URI is never shown as the main text.
        XCTAssertFalse(viewModel.torrentPrompt?.displayName.contains("magnet:") == true)
    }
    
    /// A remote .torrent prompt shows the file name and type.
    @MainActor
    func testRemoteTorrentPromptShowsFilename() {
        let viewModel = BrowserViewModel()
        viewModel.promptTorrent(url: URL(string: "https://example.com/Example.Movie.2026.torrent")!)
        XCTAssertTrue(viewModel.showTorrentPrompt)
        XCTAssertEqual(viewModel.torrentPrompt?.kind, .remoteTorrent)
        XCTAssertEqual(viewModel.torrentPrompt?.displayName, "Example.Movie.2026.torrent")
        XCTAssertEqual(viewModel.torrentPrompt?.kindLabel, "Remote torrent file")
        XCTAssertNil(viewModel.torrentPrompt?.infoHash)
        
        // Non-torrent URLs never open the torrent prompt.
        viewModel.promptTorrent(url: URL(string: "https://example.com/movie.mp4")!)
        XCTAssertEqual(viewModel.torrentPrompt?.displayName, "Example.Movie.2026.torrent")
    }
    
    // MARK: - Torrent prompt flow
    
    /// Magnet links must never reach the generic DownloadEngine — the
    /// download prompt ignores them and the torrent prompt engages instead.
    @MainActor
    func testMagnetNeverReachesDownloadEngine() {
        let viewModel = BrowserViewModel()
        let magnet = URL(string: "magnet:?xt=urn:btih:0123456789abcdef0123456789abcdef01234567&dn=Movie")!
        
        viewModel.promptDownload(url: magnet)
        XCTAssertNil(viewModel.pendingDownload)
        XCTAssertFalse(viewModel.showDownloadPrompt)
        
        viewModel.promptDownload(url: URL(string: "https://example.com/movie.torrent")!)
        XCTAssertNil(viewModel.pendingDownload)
        XCTAssertFalse(viewModel.showDownloadPrompt)
    ...
    /// Cancelling the torrent prompt clears ALL pending state.
    @MainActor
    func testTorrentPromptCancellationClearsState() {
        let viewModel = BrowserViewModel()
        viewModel.promptTorrent(url: URL(string: "magnet:?xt=urn:btih:0123456789abcdef0123456789abcdef01234567&dn=Movie")!)
        XCTAssertTrue(viewModel.showTorrentPrompt)
        
        viewModel.cancelTorrentPrompt()
        
        XCTAssertNil(viewModel.torrentPrompt)
        XCTAssertFalse(viewModel.showTorrentPrompt)
        ...
        // A fresh prompt afterwards carries no stale state.
        viewModel.promptTorrent(url: URL(string: "https://example.com/new.torrent")!)
        XCTAssertNil(viewModel.torrentPromptErrorMessage)
        XCTAssertFalse(viewModel.isTorrentPromptLoading)
        XCTAssertTrue(viewModel.showTorrentPrompt)
    }
    
    /// Repeated tap/bridge events for the same link never stack duplicate
    /// torrent prompts.
    @MainActor
    func testTorrentPromptDeduplicatesRepeatedTriggers() {
        let viewModel = BrowserViewModel()
        let magnet = URL(string: "magnet:?xt=urn:btih:0123456789abcdef0123456789abcdef01234567&dn=Movie")!
        
        viewModel.promptTorrent(url: magnet)
        let firstID = viewModel.torrentPrompt?.id
        XCTAssertNotNil(firstID)
        
        // JS bridge + navigation delegate both detect the same tap.
        viewModel.promptTorrent(url: magnet)
        viewModel.promptTorrent(url: magnet)
        XCTAssertEqual(viewModel.torrentPrompt?.id, firstID)
        XCTAssertTrue(viewModel.showTorrentPrompt)
        
        // A genuinely different link replaces the visible prompt exactly once.
    ...
    /// the prompt open with no stale success state.
    @MainActor
    func testTorrentPromptRemoteFailureShowsError() {
        let viewModel = BrowserViewModel()
        viewModel.remoteTorrentDataLoader = { _ in
            throw URLError(.badServerResponse, userInfo: [NSLocalizedDescriptionKey: "The server returned an error (HTTP 500)."])
        }
        viewModel.promptTorrent(url: URL(string: "https://example.com/broken.torrent")!)
        viewModel.startTorrentAddition()
        
        XCTAssertTrue(poll { viewModel.torrentPromptErrorMessage != nil })
        XCTAssertTrue(viewModel.showTorrentPrompt)
        XCTAssertNotNil(viewModel.torrentPrompt)
        XCTAssertFalse(viewModel.isTorrentPromptLoading)
        XCTAssertFalse(viewModel.torrentPromptSubmissionInFlight)
        XCTAssertTrue(viewModel.torrentPromptErrorMessage?.contains("HTTP 500") == true)
    }
    
    /// Torrent prompt success clears ALL pending state. Exercises the full
    /// browser → TorrentService path with a real session and a minimal valid
    /// .torrent payload served by a stubbed loader.
    @MainActor
    func testTorrentPromptSuccessClearsAllState() {
        let viewModel = BrowserViewModel()
        let service = TorrentService.shared
        viewModel.remoteTorrentDataLoader = { _ in Self.makeMinimalTorrentData() }
        
        viewModel.promptTorrent(url: URL(string: "https://example.com/success.torrent")!)
        XCTAssertTrue(viewModel.showTorrentPrompt)
        
        viewModel.startTorrentAddition()
        XCTAssertTrue(viewModel.isTorrentPromptLoading)
        
        XCTAssertTrue(poll { !viewModel.showTorrentPrompt }, "prompt must dismiss after a successful add")
        XCTAssertNil(viewModel.torrentPrompt)
        XCTAssertNil(viewModel.torrentPromptErrorMessage)
        XCTAssertFalse(viewModel.isTorrentPromptLoading)
        XCTAssertFalse(viewModel.torrentPromptSubmissionInFlight)
        
        // The torrent reached the shared TorrentService (visible in the
        // Torrent tab without an app restart). The 1 Hz session refresh
        // publishes it into `torrents` shortly after the add.
        let appeared = poll {
            service.torrents.contains { $0.name == "test" || $0.name == "browser-test" }
        }
        XCTAssertTrue(appeared, "added torrent must appear in TorrentService.torrents")
        
        // Cleanup: remove the added torrent and stop the session timer.
        if let added = service.torrents.first(where: { $0.name == "test" || $0.name == "browser-test" }) {
            service.removeTorrent(added.id, deleteFiles: true)
        }
        service.stopSession()
    }
    
    /// Minimal but valid single-file bencode torrent.
    private static func makeMinimalTorrentData() -> Data {
        var data = Data()
        func append(_ string: String) { data.append(Data(string.utf8)) }
        append("d8:announce29:http://x.example.com/announce4:infod6:lengthi1e4:name12:browser-test12:piece lengthi16384e6:pieces20:")
