...
public final class TorrentViewModel: ObservableObject {
    ...
    public func remove(_ id: String, deleteFiles: Bool) {
        ...
        // A valid removal must not leave stale selection state: when the last
        // selected torrent is gone, leave edit mode immediately.
    ...
    public func removeSelected(deleteFiles: Bool) {
        ...
        service.removeTorrents(ids, deleteFiles: deleteFiles)

        if !deleteFiles, !models.isEmpty {
        ...
        // After a valid delete/remove operation is initiated, always clear the
        // selection and leave edit mode — regardless of how many torrents
        // remain. The edit action bar must close even when only some of the
        // torrents were selected.
        selectedIDs.removeAll()
        if isEditing {
            exitEditMode()
        }
